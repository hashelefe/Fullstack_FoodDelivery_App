                ...
                <Link to={`/restaurants/${restaurant.id}`}>
                    <div className="restaurantCard" key={restaurant.id}>
                        ...
                        <div className="restaurantCardText">
                            <h3 style={{margin: 2}}>{restaurant.name}</h3>
                            <small>{restaurant.openHour} - {restaurant.closeHour} | {restaurant.rating} {star}</small>
                            <p>{restaurant.description}</p>
                            <small className='category' style={{fontSize: 11, padding:7, fontWeight: 100}}>{restaurant.category}</small>
                        </div>
                    </div>
            </Link>
            ))}
